...
# Custom Tc and Pc
def tc_pc(sg_hc):
    coefic_pc = [-12.607780250106, 670.360788329333, -0.0185900487322924]
    coefic_tc = [9.0626816494811, 183.55989816053, -0.00255420923604081]
    mw_gas = mwAir * sg_hc
...
VSHIFT = np.array([-0.27593, -0.22896, -0.21066, -0.12899, -0.19322])
...
VCVIS = np.array([1.59031, 1.60863, 1.44145, 1.20186, 0]) # cuft/lbmol    
...
def calc_bips(hc_mw, degf):                                                                     
    ...
    #                      CO2      H2S        N2        H2 
    intcpts = np.array([0.404192, 0.269304, 0.637898, 1.03807])
    mw_slopes = np.array([-0.000187046, -0.00192917, 0.000520885, 0.00644143])
    degR_slopes = np.array([-179.429, -73.9024, -338.142, -547.057])
    hc_bips = list(intcpts + degR_slopes/degR + mw_slopes * hc_mw)
    
    # Inert:Inert BIP Pairs
    #            CO2:H2S   CO2:N2   H2S:N2  CO2:H2  H2S:H2  N2:H2
    inert_bips = [0.05398, -0.23379, -0.31643, 0.50525, 0.6000, 0.2675]
...
def lbc(Z, degf, psia, sg, co2=0.0, h2s=0.0, n2=0.0, h2 = 0.0):
    ...
    def vcvis_hc(mw): # Returns hydrocarbon gas VcVis for LBC viscosity calculations   
        return  0.069352858 * mw + 0.360081275 # ft3/lbmol         
    ...
    a = [0.1023, 0.023364, 0.058533, -4.05460e-02, 9.03791e-03] # P3 and P4 have been modified
