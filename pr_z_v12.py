...
# Custom Tc and Pc
def tc_pc(sg_hc):
    v1, v2, v3 = 8.10544585018431e-05, 0.05675590493647402, 0.7649982481025761
    Offset, Pl, Vl = 11.188806249822868, 183.5258567664124, 2654.6738696902653
...
VCVIS = np.array([1.43556, 1.45055, 1.33560, 0.75786, 0]) # cuft/lbmol    
...
def calc_bips(hc_mw, degf):                                                                     
    ...
    #                      CO2      H2S        N2        H2 
    intcpts = np.array([0.397871, 0.281553, 0.509440, 0.809360])
    mw_slopes = np.array([-0.00276205, -0.00441606, -0.00437519, 0.00884004])
    degR_slopes = np.array([-157.440, -61.19800, -222.582, -470.398 ])

    hc_bips = list(intcpts + degR_slopes/degR + mw_slopes * hc_mw)
    
    # Inert:Inert BIP Pairs
    #            CO2:H2S       CO2:N2     H2S:N2    CO2:H2  H2S:H2  N2:H2
    inert_bips = [0.057084, -0.220628, -0.195055, 0.648104, 0.65, 0.36917]
...
def lbc(Z, degf, psia, sg, co2=0.0, h2s=0.0, n2=0.0, h2 = 0.0):
    ...
    def vcvis_hc(mw): # Returns hydrocarbon gas VcVis for LBC viscosity calculations   
        return  0.0577172 * mw + 0.480478182 # ft3/lbmol      
    ...
    a = [0.1023, 0.023364, 0.058533, -3.88177e-02,  9.13868e-03] # P3 and P4 have been modified
