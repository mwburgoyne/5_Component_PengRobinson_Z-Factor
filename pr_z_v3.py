...
def tc_pc(sg_hc):
    x = [-3.55897620e-03, -2.78434498e+00,  7.04262503e+02, -1.00841444e-01, 1.17914323e+01,  1.80343787e+02]
    coefic_pc = x[:3]
    coefic_tc = x[3:]
    mw_gas = mwAir * sg_hc
...
ACF = np.array([0.19001, 0.09, 0.037, -0.04896])
VSHIFT = np.array([-0.09348, -0.12435, -0.20976, -0.3949])
OmegaA = np.array([0.454159, 0.457236, 0.457236, 0.429188])
OmegaB = np.array([0.0768737, 0.0777961, 0.0777961, 0.0692551]) 
...
def calc_bips(hc_mw, degf):
    ...
    intcpts = np.array([8.42121e-01, -3.04689e-02, -1.44878e+00])
    degR_slopes = np.array([7.16203e+01, 4.56533e+01, 1.36500e+02])
    mw_slopes = np.array([-5.22940e-02, 5.36664e-03, 4.26866e-02])
    bips = intcpts + degR_slopes/degR + mw_slopes * hc_mw
    
    # Inert-Inert BIPS (Regressed to GERG2008 predictions)
    BIP_HC = 7.2887 * degf**-0.833341997
    BIP_CN = -0.314354505 + (degf * 0.320886621) / (85.36118324 + degf)
    BIP_HN = -0.74234267 + (degf * 3.333517273) / (485.3007919 + degf)
...
def lbc(Z, degf, psia, sg, co2=0.0, h2s=0.0, n2=0.0):
    ...
    def vcvis_hc(mw): # Returns hydrocarbon gas VcVis for LBC viscosity calculations
        return  0.055267432 * mw + 0.461335667 # ft3/lbmol
    ...
    a = [0.1023, 0.023364, 0.058533, -3.45450e-02, 7.43508e-03] # P3 and P4 have been modified
